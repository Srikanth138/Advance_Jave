...
public class FileUplodServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			// create Special Request object which holds form data and upload files as
			// streams
			MultipartFormDataRequest nreq = new MultipartFormDataRequest(req);

			// read form data from input file
			...
			resumeFilePath = resumeLocation + ((UploadParameters) vector.get(0)).getFilename(); // gives
																								// E:/upload/resumes/resume.txt
			photoFilePath = photoLocation + ((UploadParameters) vector.get(1)).getFilename(); // gives
																								// E:/upload/photos/photo.jpg
			...
			uploadFlag = false;
			pw.println("<h1 style='color:red'>Problem in file uploading-->" + e.getMessage() + "</h1>");
			e.printStackTrace();
		}
		if (uploadFlag) {
			// write JDBC code..
