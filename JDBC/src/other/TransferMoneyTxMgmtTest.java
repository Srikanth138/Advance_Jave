...
public class TransferMoneyTxMgmtTest {
	public static void main(String[] args) {
		...
		try {
			...
			//begin Tx
			if(con!=null)
				con.setAutoCommit(false); // not save detail's directly
			//add withdraw , deposite queries to the batch
			if(st!=null) {
				//for withdraw operation
				st.addBatch("UPDATE  JDBC_BANKACCOUNT SET AMT=AMT-"+amt+ "WHERE ACNO="+srcAcno);
				//for deposiste operation
				st.addBatch("UPDATE  JDBC_BANKACCOUNT SET AMT=AMT+"+amt+ "WHERE ACNO="+destAcno);
			}
			 //execut batch
			if(st!=null) 
