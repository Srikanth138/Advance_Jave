...
//		Connection con = DBConnectionUtil.getPooledConnection();
		try (Connection con = DBConnectionUtil.getPooledConnection()){ // 1 try with resource
			// create PreparedStatement object
			//PreparedStatement ps = con.prepareStatement(GET_BOOKS_BY_CATEGORY);
			try (PreparedStatement ps = con.prepareStatement(GET_BOOKS_BY_CATEGORY)) {// 2 try with resource
				// set value to qyery params
				ps.setString(1, category);
				// send and execute SQL query
				//ResultSet rs = ps.executeQuery();
				try (ResultSet rs = ps.executeQuery()) { // 3 try with resource
					// copy RS records to ArrayList
					listBO = new ArrayList();
					while (rs.next()) {
						// copy each record of RS to One Object of BO class
