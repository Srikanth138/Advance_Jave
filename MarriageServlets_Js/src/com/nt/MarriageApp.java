...
public class MarriageApp extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if (vflag.equalsIgnoreCase("no")) { // it will execute when javascript code is not execute in client side web
											// browser is disable
			...
			if (!errList.isEmpty()) {
				/*
				 * Iterator<String> it=errList.iterator(); pw.println(it.hashCode());
				 */

				errList.forEach(msg -> { // multiple line
					pw.println("<li><span style='color:red'>" + msg + "</span></li>");
				}); // forEach() method
				return; // it not return any think becase it return type is void. but it will stop the
						// reaming code execution.
			} // if
		} // if
		else {
		...
		if (gender.equalsIgnoreCase("male")) {
			age = Integer.parseInt(age1);
			if (age >= 21) {
				pw.println("<h1 style='color:red; text-align:center;'>Welcome YouR Eligible for Marrige Mr." + name
						+ " Best of luck" + age + ", " + gender + "<h1>");
			} else {
				pw.println("<h1 style='color:green; text-align:center;'>YouR Not Eligible for Marrige Mr." + name
						+ "</br> Wait for " + (21 - age) + "Year " + gender + "</h1>");
			}
		}
			...
			age = Integer.parseInt(age1);
			if (age >= 18) {
				pw.println("<h1 style='color:red; text-align:center;'> Welcome YouR Eligible for Marrige Miss." + name
						+ " Best of luck" + age + ", " + gender + "</h1>");
			} else {
				pw.println("<h1 style='color:green; text-align:center;'>YouR Not Eligible for Marrige Miss." + name
						+ "</br> Wait for " + (18 - age) + "Year " + gender + "</h1>");
			}
		}
		pw.println("</center>");
